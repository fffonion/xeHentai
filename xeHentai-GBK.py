...
if __name__=='__main__':
    try:
        ...
        while True:
            ...
            if not (exurl_all.startswith('http://g.e-hentai.org/') or exurl_all.startswith('http://exhentai.org/')):prompt('咦?这是啥')
            else:
                if exurl_all:break
                else:prompt('必须输入地址~')
        ...
        for exurl in exurl_all:
            ...
            if opth.exists(opth.join(getPATH0(),legalpath(gname)+'.txt')) and getdowntype()=='full':#非完整图已变成509
